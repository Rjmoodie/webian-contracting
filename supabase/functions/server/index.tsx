...
// Get service types
app.get("/server/services", async (c) => {
  try {
...
app.get("/server/client-ratings", async (c) => {
  try {
...
app.get("/server/lookups", async (c) => {
  try {
...
app.post("/server/requests", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
...
app.get("/server/requests", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
// Get single project
app.get("/server/requests/:id", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
app.post("/server/requests/:id/attachments", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
app.put("/server/requests/:id/status", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
// Add note to project
app.post("/server/requests/:id/notes", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
// Cancel project
app.post("/server/requests/:id/cancel", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
app.get("/server/requests/:id/activity", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
app.post("/server/requests/:id/quote", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
    if (profile.role !== 'admin' && profile.role !== 'manager') {
...
app.post("/server/requests/:id/quote/accept", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
app.post("/server/requests/:id/quote/reject", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
app.get("/server/admin/services", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
    if (profile.role !== 'admin' && profile.role !== 'manager') {
...
app.post("/server/services", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
    if (profile.role !== 'admin') return c.json({ error: "Forbidden" }, 403);
...
app.get("/server/admin/talents", async (c) => {
  return c.json({ talents: [] });
});

app.get("/server/admin/talent/pending", async (c) => {
  return c.json({ talents: [] });
});

app.get("/server/portfolio", async (c) => {
  return c.json({ items: [] });
});

app.get("/server/services/:id", async (c) => {
  try {
...
app.get("/server/requests/:id/messages", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
// Send a new message
app.post("/server/requests/:id/messages", async (c) => {
  try {
    const { profile, error } = await getAuthUser(c);
    if (error || !profile) return c.json({ error: "Unauthorized" }, 401);
...
// ════════════════════════════════════════════
app.post("/server/webhooks/inbound-email", async (c) => {
  try {
